...
import { TIER_LABEL } from "@/lib/data/types";
    ...
    return (
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <p className="mt-2 text-[0.85rem] text-[var(--muted-on-ink)]">
          We&apos;ll email {email} when {TIER_LABEL[tier]} opens up in {country}. No spam, just launch
          news.
        </p>
      </div>
    );
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <select
          ...
          className="w-full rounded-full border border-white/15 bg-white/5 px-4 py-3 text-sm text-[var(--paper-text)] focus:border-[var(--flame)] focus:outline-none [&>option]:text-black"
        >
          {TIER_OPTIONS.map((t) => (
...
function InvestorForm() {
  const { submit } = useInvestors();
  const [status, setStatus] = useState<"idle" | "loading" | "done">("idle");
  const [name, setName] = useState("");
  const [fund, setFund] = useState("");
  const [email, setEmail] = useState("");
  const [requestedAmount, setRequestedAmount] = useState("$250k–$500k");
  ...
  function handleSubmit(e: FormEvent) {
    ...
    setStatus("loading");
    setTimeout(() => {
      submit({ name, fund, email, requestedAmount, wantsDemo });
      setStatus("done");
    }, 700);
  }
    ...
    return (
      <div className="rounded-2xl border border-[var(--paper-line)] bg-white/60 p-6">
        ...
        <p className="mt-2 text-[0.85rem] text-[var(--muted-on-paper)]">
          {wantsDemo
            ? "The deck, data room link, and a demo time will be in your inbox within one business day."
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      ...
      <select
        ...
        className="w-full rounded-full border border-[var(--paper-line)] bg-white/70 px-5 py-3 text-sm text-[var(--ink-text)] focus:border-[var(--flame-dim)] focus:outline-none"
      >
        {["$100k–$250k", "$250k–$500k", "$500k–$1M", "$1M–$2M", "$2M–$5M", "$5M+"].map((a) => (
      ...
      <label className="flex items-center gap-2 px-2 text-[0.83rem] text-[var(--muted-on-paper)]">
        <input
          type="checkbox"
          checked={wantsDemo}
          onChange={(e) => setWantsDemo(e.target.checked)}
          className="h-4 w-4 rounded border-[var(--paper-line)] accent-[var(--flame)]"
        />
        I&apos;d like a live product demo
      </label>
      ...
      <section id="investors" className="bg-[var(--paper-soft)] py-24 sm:py-28">
        <div className="mx-auto max-w-3xl px-6 text-center">
          ...
          <p className="mx-auto mt-5 max-w-md text-[0.95rem] leading-relaxed text-[var(--muted-on-paper)]">
            Leave your details and we&apos;ll send the full deck, data room access,
            and a time to talk.
          </p>
          <div className="mx-auto mt-9 max-w-md">
            <InvestorForm />
